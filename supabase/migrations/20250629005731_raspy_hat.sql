/*
  # Create dummy data insertion function

  1. New Functions
    - `insert_dummy_data()` - Creates comprehensive test data for all tables
    - Uses the authenticated user's ID from auth.uid()
    - Includes realistic healthcare data across all tables
    
  2. Security
    - Function respects RLS policies
    - Only creates data for the authenticated user
...
-- Create function to insert comprehensive dummy data
...
BEGIN
    ...
    IF existing_patients_count > 0 THEN
        RETURN 'User already has ' || existing_patients_count || ' patients. Dummy data not inserted to avoid duplicates.';
    END IF;
    ...
    RETURN 'Successfully inserted dummy data: 8 patients, 5 providers, 8 appointments, 6 pre-authorizations, 6 intake tasks, 8 insurance eligibility records, 8 insurance records, 4 document templates, 6 patient documents, 8 notifications, and 5 audit log entries.';
...
COMMENT ON FUNCTION public.insert_dummy_data() IS 'Inserts comprehensive dummy data for testing purposes. Only works for authenticated users and prevents duplicate data insertion.';