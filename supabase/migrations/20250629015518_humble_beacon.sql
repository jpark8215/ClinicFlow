/*
  # Fix RLS Policies for Clinic-Wide Data Access

  1. Security Changes
    - Remove user-specific RLS restrictions
    - Allow all authenticated users to see all clinic data
    - Maintain data integrity while enabling collaboration

  2. Policy Updates
    - Update all table policies to allow clinic-wide access
    - Keep audit trails for accountability
    - Ensure proper data isolation only at the clinic level (not user level)
*/

-- Drop existing restrictive RLS policies and create clinic-wide access policies

-- Patients table - Allow all authenticated users to access all patient data
...
-- Providers table - Allow all authenticated users to access all provider data
DROP POLICY IF EXISTS "Users can manage their own providers" ON providers;

CREATE POLICY "Clinic staff can manage all providers"
  ON providers
  FOR ALL
  TO authenticated
  USING (true)
...
-- Appointments table - Allow all authenticated users to access all appointment data
...
-- Pre-authorizations table - Allow all authenticated users to access all preauth data
DROP POLICY IF EXISTS "Users can delete their own pre-authorizations" ON pre_authorizations;
DROP POLICY IF EXISTS "Users can insert their own pre-authorizations" ON pre_authorizations;
DROP POLICY IF EXISTS "Users can update their own pre-authorizations" ON pre_authorizations;
DROP POLICY IF EXISTS "Users can view their own pre-authorizations" ON pre_authorizations;

CREATE POLICY "Clinic staff can manage all pre-authorizations"
...
-- Intake tasks table - Allow all authenticated users to access all intake data
DROP POLICY IF EXISTS "Users can manage their own intake tasks" ON intake_tasks;

CREATE POLICY "Clinic staff can manage all intake tasks"
  ON intake_tasks
  FOR ALL
  TO authenticated
  USING (true)
...
-- Insurance eligibility table - Allow all authenticated users to access all eligibility data
DROP POLICY IF EXISTS "Users can manage their own eligibility records" ON insurance_eligibility;

CREATE POLICY "Clinic staff can manage all insurance eligibility"
  ON insurance_eligibility
  FOR ALL
  TO authenticated
  USING (true)
  WITH CHECK (true);

-- Patient insurance table - Allow all authenticated users to access all insurance data
DROP POLICY IF EXISTS "Users can manage their patients insurance" ON patient_insurance;

CREATE POLICY "Clinic staff can manage all patient insurance"
...
-- Appointments providers table - Allow all authenticated users to access all appointment-provider relationships
DROP POLICY IF EXISTS "Users can manage their appointment providers" ON appointments_providers;

CREATE POLICY "Clinic staff can manage all appointment providers"
...
-- Document templates table - Allow all authenticated users to access all templates
DROP POLICY IF EXISTS "Users can manage their own document templates" ON document_templates;

CREATE POLICY "Clinic staff can manage all document templates"
...
-- Patient documents table - Allow all authenticated users to access all patient documents
DROP POLICY IF EXISTS "Users can manage their patients documents" ON patient_documents;

CREATE POLICY "Clinic staff can manage all patient documents"
...
-- Notifications table - Allow all authenticated users to access all notifications
DROP POLICY IF EXISTS "Users can manage their own notifications" ON notifications;

CREATE POLICY "Clinic staff can manage all notifications"
  ON notifications
  FOR ALL
  TO authenticated
  USING (true)
...
-- Audit logs table - Allow all authenticated users to view all audit logs (read-only for transparency)
DROP POLICY IF EXISTS "Users can view their own audit logs" ON audit_logs;

CREATE POLICY "Clinic staff can view all audit logs"
  ON audit_logs
  FOR SELECT
  TO authenticated
  USING (true);

-- Keep audit log inserts restricted to system functions only
...
-- Update the dummy data function to not filter by user_id
...
Provider: _______________', 
    ...
    RETURN 'Successfully inserted clinic-wide dummy data: 8 patients, 5 providers, 8 appointments, 6 pre-authorizations, 6 intake tasks, 8 insurance eligibility records, 8 insurance records, 4 document templates, 6 patient documents, 8 notifications, and 5 audit log entries. All data is now accessible to all clinic staff.';
