/*
  # Create function to insert dummy data for authenticated users

  This migration creates a function that can be called by authenticated users
  to populate their account with realistic test data for development and testing.
*/

-- Create a function to insert dummy data for the current authenticated user
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM public.patients WHERE user_id = current_user_id LIMIT 1) THEN
        RETURN 'INFO: User already has data. Skipping dummy data insertion.';
    END IF;
    ...
    RETURN 'SUCCESS: Dummy data inserted successfully for user ' || current_user_id::text || '. Created 8 patients, 5 providers, 8 appointments, 6 pre-authorizations, 6 intake tasks, 8 insurance eligibility records, 8 insurance records, 4 document templates, 6 patient documents, 8 notifications, and 5 audit log entries.';
...
COMMENT ON FUNCTION public.insert_dummy_data() IS 'Call this function while authenticated to populate your account with realistic test data. Usage: SELECT public.insert_dummy_data();';