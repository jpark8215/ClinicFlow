/*
  # Direct Dummy Data Insertion

  This migration directly inserts comprehensive dummy data for all tables.
  It bypasses authentication checks and uses a fixed user ID that will be
  created if it doesn't exist.

  ## What this creates:
  1. A test user in auth.users (if needed)
  2. 8 patients with complete profiles
  3. 5 healthcare providers
  4. 8 appointments with various statuses
  5. 6 prior authorizations
  6. 6 intake tasks
  7. 8 insurance eligibility records
  8. 8 patient insurance records
  9. 4 document templates
  10. 6 patient documents
  11. 8 notifications
  12. 5 audit log entries
*/

-- Create a test user ID that we'll use for all data
DO $$
DECLARE
...
BEGIN
    ...
    SELECT COUNT(*) INTO existing_count FROM public.patients WHERE user_id = test_user_id;
    
    IF existing_count > 0 THEN
        RAISE NOTICE 'Dummy data already exists for user %. Skipping insertion.', test_user_id;
        RETURN;
    END IF;
...
Provider: _______________', 
    ...
    RAISE NOTICE 'Successfully inserted dummy data: 8 patients, 5 providers, 8 appointments, 6 pre-authorizations, 6 intake tasks, 8 insurance eligibility records, 8 insurance records, 4 document templates, 6 patient documents, 8 notifications, and 5 audit log entries for user %.', test_user_id;

EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION 'Error inserting dummy data: %', SQLERRM;
END $$;
